    ...
    <div class="user">
      <div class="top">
        ...
        <div class="switch">
          <div
            class="switch-item"
            :class="{ active: selected === 'mylikeSongs' }"
            @click="switchItem('mylikeSongs')"
          >
            我喜欢的
          </div>
          <div
            class="switch-item"
            :class="{ active: selected === 'playHistory' }"
            @click="switchItem('playHistory')"
          >
            最近听的
          </div>
        </div>
      ...
      <div class="container-item" v-show="selected === 'mylikeSongs'">
        ...
        <div
          class="play-btn"
          v-show="mylikeSongs.length"
          @click="randomPlay(mylikeSongs, 'mylikeSongs')"
        >
        ...
        <ul class="list">
          <li
            v-for="(item, index) in mylikeSongs"
            :key="index"
            class="item"
            @click="selectItem(mylikeSongs, index, 'mylikeSongs')"
          >
            <h3
              v-html="item.songname"
              :class="active(item, 'mylikeSongs')"
            ></h3>
            <p
              v-html="item.singername + '·' + item.albumname"
              :class="active(item, 'mylikeSongs')"
            ></p>
        ...
        <div
          @click="clear('mylikeSongs')"
          v-show="mylikeSongs.length >= 7"
          class="clearlist"
        >
      ...
      <div class="container-item" v-show="selected === 'playHistory'">
        ...
        <div
          class="play-btn"
          v-show="playHistory.length"
          @click="randomPlay(playHistory, 'playHistory')"
        >
          <i class="iconfont icon-play_icon"></i>
          <span>随机播放全部</span>
        </div>
        <ul class="list">
          <li
            ...
            :key="index"
            class="item"
            @click="selectItem(playHistory, index, 'playHistory')"
          >
            <h3
              v-html="item.songname"
              :class="active(item, 'playHistory')"
            ></h3>
            <p
              v-html="item.singername + '·' + item.albumname"
              :class="active(item, 'playHistory')"
        ...
        <div
          @click="clear('playHistory')"
          v-show="playHistory.length >= 7"
          class="clearlist"
        >
          <i class="iconfont icon-lajitong1"></i>&ensp;清空列表
        </div>
...
export default {
  ...
  computed: {
    ...mapState(["mylikeSongs", "playHistory", "playlist", "mode", "other"]),
